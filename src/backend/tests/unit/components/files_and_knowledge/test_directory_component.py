...
class TestDirectoryComponent(ComponentTestBaseWithoutClient):
    ...
    def test_directory_blocked_outside_storage_when_restricted(self, tmp_path):
        ...
        with patch("lfx.utils.file_path_security.get_settings_service", return_value=settings_mock):
            ...
            # An absolute path outside the storage dir is blocked.
            blocked = DirectoryComponent()
            blocked.set_attributes(
                {"path": "/etc", "use_multithreading": False, "silent_errors": False, "types": ["txt"]}
            )
