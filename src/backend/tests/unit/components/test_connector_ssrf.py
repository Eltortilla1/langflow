...
to assert it is never reached) — the real ``validate_url_for_ssrf`` logic runs.

The vector-store connector guards (qdrant/weaviate/elasticsearch/opensearch/milvus/supabase/
upstash/clickhouse/chroma) and astradb_cql use the identical one-line ``validate_url_for_ssrf``
call, which is exercised directly in ``lfx/tests/unit/utils/test_ssrf_protection.py``.
"""
...
def test_deepseek_get_models_blocks_metadata_without_request():
    ...
    with ssrf_enabled(), patch("requests.get") as mock_get:
...
def test_litellm_proxy_blocks_metadata_without_request():
    ...
    with (
        ssrf_enabled(),
        patch("lfx.components.litellm.litellm_proxy.httpx.get") as mock_get,
...
def test_huggingface_inference_endpoint_blocks_metadata_without_request():
    ...
    with (
        ssrf_enabled(),
        patch("lfx.components.huggingface.huggingface_inference_api.requests.get") as mock_get,
        pytest.raises(ValueError, match="SSRF"),
    ):
...
def test_glean_blocks_metadata_before_token_sent():
    ...
    with ssrf_enabled(), pytest.raises(SSRFProtectionError):
        wrapper._prepare_request("query")
